...
export function ChatKitPanel({
  ...
  const setErrorState = useCallback((updates: Partial<ErrorState>) => {
    setErrors((cur) => ({ ...cur, ...updates }));
  }, []);

  useEffect(() => () => { isMountedRef.current = false; }, []);

  // ChatKit script loader
  useEffect(() => {
    if (!isBrowser) return;
        ...
        if (!window.customElements?.get("openai-chatkit")) {
          handleError(
            new CustomEvent("chatkit-script-error", {
              detail: "ChatKit script unavailable.",
            })
          );
        }
      }, 5000);
  ...
  const getClientSecret = useCallback(
    async (currentSecret: string | null): Promise<string> => {
      ...
      try {
        ...
        const msg =
          err instanceof Error ? err.message : "Failed to create ChatKit session.";
        setErrorState({ session: msg, retryable: false });
        throw new Error(msg);
      } finally {
        if (isMountedRef.current && !currentSecret) {
  ...
  const chatkit = useChatKit({
    ...
    onClientTool: async (invocation: {
      ...
      // Human Handoff + Progressive Profile
      if (invocation.name === "handoff_to_slack") {
        ...
        try {
          ...
          return {
            success: true,
            message:
              type === "progressive_profile"
                ? "Thanks — your profile details were updated."
                : "Thanks! Your details were sent to the Azence team.",
          };
        } catch (err) {
          console.error("Slack handoff failed:", err);
          return {
            success: false,
            message: "Something went wrong while sending your details.",
          };
        }
      }
